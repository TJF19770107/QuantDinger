...
## Anthropic官方课程R88同步：多Agent生态与架构设计原则（2026-06-16）

### Claude Code多Agent生态四大组件

| 组件 | 角色 | 类比 | 核心定位 |
|------|------|------|---------|
| **Skills** | 专业知识库 | 操作手册 | 教会Agent怎么做特定类型的事 |
| **MCP** | 外部连接器 | 通往数据库/API/工具的桥梁 | 让Agent能连接外部世界 |
| **Subagents（子代理）** | 干活的工人 | 外包的自由职业者 | 并行干活，隔离上下文 |
| **Agent Teams** | 协作的团队 | 能互相沟通的项目组 | 多代理间协商、互相验证 |

> 核心公式：**Skills（操作手册）→ MCP（外部连接）→ 子代理（并行干活）→ Agent Teams（协作协商）**

### 子代理上下文隔离机制

子代理最大的价值不是并行速度，而是**上下文隔离**：
...
子代理 Context Window（独立，不污染主对话）
    ├── 完整执行过程
    ├── 工具调用链路
    └── 中间推理步骤
```

> 核心洞察：「所有中间噪音留在子代理的上下文里，主代理只拿到信号。」

### Agent Teams 核心架构

**四组件**：
- **Team Lead**：创建团队、生成队友并协调工作的主会话。仅使用协调工具时可激活委派模式
- **Teammates**：独立 Claude Code 实例，各自拥有上下文窗口、工具访问权和对话历史
- **Shared Task List**：存储在 `~/.claude/tasks/{team-name}/`，支持 pending/in_progress/completed 状态、依赖关系和自认领
- **Mailbox System**：任意代理间直接点对点通信，支持直接消息和广播

**对比 Subagents**：

| 维度 | Subagents | Agent Teams |
|------|-----------|-------------|
| 通信 | 仅向主Agent报告 | 队友直接相互发送消息 |
| 协调 | 主Agent管理所有工作 | 共享任务列表自协调 |
| 持久性 | 任务范围 | 完整会话 |
| Token成本 | 1.5-2x | 3-4x |

### Managed Agents 四件套（2026年5月更新）

| 能力 | 状态 | 核心价值 |
|------|------|---------|
| **Dreaming（自我改进）** | 研究预览 | 离回访过去任务、提取模式、整理记忆，让Agent越用越好 |
| **Outcomes（自动质量把关）** | 已开放 | 独立评分器评估产出，未达标时自动迭代修正，成功率最高提升10个百分点 |
| **Multiagent Orchestration** | 已开放 | 主代理拆解任务、派发给不同模型/提示/工具的专家代理并行执行 |
| **Webhooks（事件驱动）** | 已开放 | Agent完成任务后主动通知外部系统，从"对话式"变为"事件驱动" |

### 委派模式（Delegation Mode）

委派模式下 Team Lead 仅保留协调工具权限，**不能直接读文件、编辑代码或运行命令**。所有实现工作必须通过队友进行。

**适用场景**：4个以上队友的大团队，强制实现与协调分离，产生更好的任务分解和更清晰的所有权边界。
