...
## 子代理管理规范（来源：Anthropic Academy - Introduction to Subagents）

### 子代理配置模板

```yaml
name: "subagent_name"              # 子代理唯一标识
description: "子代理职责描述"       # 清晰定义子代理的职责范围
tools:                            # 授权工具列表
  - read_text
  - search_file
  - shell_executor
context:                          # 上下文配置
  max_tokens: 32000
  auto_compact: true
output_format: "markdown"         # 输出格式约定（markdown/json/structured）
```
...
| 阶段 | 操作 | 说明 |
|------|------|------|
| 1. **创建（Create）** | 定义子代理配置文件和提示词 | 通过 `/agents` 命令或 `.claude/agents/` 目录创建 `.md` 配置文件 |
| 2. **委托（Delegate）** | 主 Agent 将任务派发给子代理 | 结构化指令包含目标、上下文、工具权限、输出格式 |
| 3. **执行（Execute）** | 子代理独立完成任务 | 子代理拥有独立上下文窗口，执行过程不污染主 Agent |
| 4. **返回（Return）** | 子代理将结果返回主 Agent | 精炼的结构化结果回传，仅回传必要信息 |
| 5. **评估（Evaluate）** | 主 Agent 评估子代理输出质量 | 检查输出完整性、准确性、格式符合性 |

### 子代理管理法则

1. **最小权限原则**：仅授予子代理完成任务所需的最少工具
2. **自包含原则**：子代理提示词必须包含所有上下文，不依赖主 Agent 隐式知识
3. **结构化输出原则**：子代理必须返回可解析的结构化结果
4. **失败上报原则**：子代理遇到无法解决的问题时，上报而非静默失败
5. **上下文效率原则**：子代理仅在收益 > 20%-50% Token 开销时使用

### 龙虾体系子代理映射

| Anthropic Subagent | 龙虾 Worker | 配置文件路径 |
|--------------------|------------|------------|
| 代码审查员 | Codex Agent | `子Agent/Codex/AGENTS.md` |
| 文件整理助手 | File Agent | `技能库/FileOrganizer.md` |
| 文档生成器 | Document Writer | 技能 `document-writer` |
| 知识蒸馏器 | Hermes Agent | `子Agent/Hermes Agent/` |
| 浏览器操作员 | Browser Agent | MCP 工具链 |
...
> **版本**：v2.0（R20 四大核心模块标准化 + 自动检索规则强化）
> **最后更新**：2026-06-01
> **下次审查**：R21 迭代
