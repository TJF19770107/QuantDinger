...
```json
{
  ...
  "version": "4.7",
  "created": "2026-05-31",
  "updated": "2026-06-01T19:00:00+08:00",
  "status": "active",
  "last_iteration_report": "LAB_USDT永续合约交易分析报告_20260601_全域缺口专项补_v4.7.md",
  "type": "breakout_momentum",
  "market": "perpetual_contract",
  "description": "龙虾黑马策略 v4.7 — 突破型动量策略，捕捉价格突破关键阻力/支撑后的持续性动量行情，结合多因子融合信号与严格风控体系。v4.7基于美伊战争黑天鹅（BTC $80K→$73K暴跌8.75%，恐惧指数29，ETF连续9日流出$28亿）叠化：新增战争黑天鹅熔断（大国军事冲突→全账户暂停新开仓）、恢复期强化（10→14天+7天模拟交易期+抄底禁止令）、仓位渐进恢复机制（2%→5%→10%）、回撤阈值收紧（日5→3%/周10→7%/月15→10%）、战争ATR倍数（2.5→3.5）、恐惧脱敏预警。保留v4.6全部ETF流出熔断、恐惧指数分层、RSI上限等约束。",
  "design_philosophy": {
  ...
  "entry_rules": {
    ...
    "signal_grade": {
      "A_grade": "主突破条件 + 5因子中≥4达标 + 布林带不超30% → 标准仓位",
      "B_grade": "主突破条件 + 5因子中≥3达标 + 布林带不超30% → 观望（不入场）",
      "C_grade": "主突破条件 + 2因子或以下 或 布林带超30% → 不入场"
    },
    ...
    "extreme_market_circuit_breaker": {
      ...
      "bollinger_extreme_no_entry": {
        ...
        "description": "v4.6优化：阈值从30%收紧至25%，LAB极端行情显示30%仍过于激进"
      }
    },
  ...
  "lab_usdt_tuning": {
    ...
    "trailing_activation_threshold": 0.02,
    "rsi_upper_altcoin": 75,
    "bollinger_pause_threshold": 0.25,
    "bollinger_period_altcoin": 10,
    "no_naked_short": true,
    "profit_ratio_repair_algorithm": "enabled",
    "max_single_loss_pct": 0.05,
    "profit_ratio_hard_constraint": 1.5,
    "current_signal": "C级 — 不参与（三重熔断+爆仓恢复期Day1/7）",
    "current_signal_detail": "EMA趋势✅ / 成交量✅ / RSI❌(>90) / MACD⚠️收窄 / 布林带❌(超上轨46%) + 贪婪熔断🔴 + 波动率熔断🔴 + 布林带极端外溢🔴",
  ...
  "current_optimized_values": {
    ...
    "post_liquidation_max_leverage": 0.5,
    "post_liquidation_simulation_days": 7,
    ...
    "drawdown_monthly_limit": 0.10,
    "fear_desensitization_warning_enabled": true,
    "position_progressive_recovery_tiers": [0.02, 0.05, 0.10],
    "position_progressive_recovery_conditions": ["sim_period_win", "5_consecutive_win", "10_consecutive_win_war_over"],
    "stop_loss_code_enforced": true,
    "stop_loss_api_required": true,
    "buy_trend_confirmation_bars": 2,
    "profit_ratio_extreme_override": 2.0,
